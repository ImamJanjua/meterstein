...
const Sparren = () => {
  ...
  const [sparrenart, setSparrenart] = React.useState("");
...
Gesendet über Meterstein
    ...
    try {
      ...
      const result = await MailComposer.composeAsync({
        ...
        subject: `Bestellung - Sparren - ${nameKunde}`,
        body: emailBody,
      ...
      >
        <View className="gap-8 p-4 bg-background/30">
          <View className="mt-8 items-center">
            <Text className="text-3xl font-bold text-red-500">Sparren</Text>
          </View>
          ...
          >
            <Image
              source={require("~/assets/images/sparren-main.webp")}
              contentFit="contain"
          ...
          <View className="gap-2">
            ...
            <RadioGroup
              ...
              className="gap-3"
            >
              {sparrenartOptions.map((option) => (
                <RadioGroupItemWithLabel
                  key={option}
                  value={option}
                  onLabelPress={() => setSparrenart(option)}
                />
              ))}
            </RadioGroup>
          </View>
