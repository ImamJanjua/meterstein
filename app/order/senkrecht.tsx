...
const Senkrecht = () => {
  ...
  // Kabeldurchführung state
  const [kabeldurchfuehrung, setKabeldurchfuehrung] = React.useState("");

  // Montageart state
  const [montageart, setMontageart] = React.useState("");
  ...
  const kabeldurchfuehrungOptions = ["1", "2", "3", "4", "5", "6", "7"];

  const montageartOptions = ["Wandmontage", "Deckenmontage"];
  ...
  async function sendOrder() {
    ...
    const emailBody = `
Bestellung - Senkrecht mit ZIP

Kundenname: ${nameKunde}

Maße:
a: ${measurementA} mm (Außenkante - Außenkante)
...
Gesendet über Meterstein
    ...
    try {
      ...
      const result = await MailComposer.composeAsync({
        ...
        subject: `Bestellung - Senkrecht mit ZIP - ${nameKunde}`,
        body: emailBody,
      });
    ...
    >
      ...
      >
        <View className="gap-4 p-4 bg-background/30">
          ...
          >
            <Image
              source={require("~/assets/images/senkrecht-main.webp")}
              contentFit="contain"
          ...
          <View className="gap-3 p-4 bg-orange-50 dark:bg-orange-950/20 border border-orange-200 dark:border-orange-800 rounded-lg">
            <Text className="text-lg font-bold text-orange-800 dark:text-orange-200">
              WICHTIG hier ist alles ausßenkante zu ausßenkante
            </Text>
          </View>
          ...
          <View className="gap-2">
            ...
            <RadioGroup
              ...
              className="gap-3"
            >
              {kabeldurchfuehrungOptions.map((option) => (
                <RadioGroupItemWithLabel
                  key={option}
                  value={option}
                  onLabelPress={() => setKabeldurchfuehrung(option)}
                />
              ))}
            </RadioGroup>
          </View>
          ...
          <View className="gap-2">
            ...
            <RadioGroup
              ...
              className="gap-3"
            >
              {montageartOptions.map((option) => (
                <RadioGroupItemWithLabel
                  key={option}
                  value={option}
                  onLabelPress={() => setMontageart(option)}
                />
              ))}
            </RadioGroup>
          </View>
