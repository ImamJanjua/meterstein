...
const Led = () => {
  ...
  // Bestellen
  const [bestellen, setBestellen] = React.useState("");

  const bestellenOptions = ["ja"];
...
Bestellen?: ${bestellen || "Nicht ausgewählt"}
      ...
      >
        <View className="gap-8 p-4 bg-background/30">
          <View className="mt-8 items-center">
            <Text className="text-3xl font-bold text-red-500">LED</Text>
          </View>
          ...
          >
            <Image
              source={require("~/assets/images/led.webp")}
              contentFit="contain"
          ...
          <View className="gap-2">
            ...
            <RadioGroup
              value={bestellen}
              onValueChange={setBestellen}
              className="gap-3"
            >
              {bestellenOptions.map((option) => (
                <RadioGroupItemWithLabel
                  key={option}
                  value={option}
                  onLabelPress={() => setBestellen(option)}
                />
              ))}
            </RadioGroup>
          </View>
