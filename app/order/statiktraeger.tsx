...
const Statikträger = () => {
  ...
  // Color and deckel set
  const [farbe, setFarbe] = React.useState("");
  const [deckelSet, setDeckelSet] = React.useState("");
  ...
  const deckelSetOptions = ["ja", "nein"];
...
Gesendet über Meterstein
    ...
    try {
      ...
      const result = await MailComposer.composeAsync({
        ...
        subject: `Bestellung - Statikträger - ${nameKunde}`,
        body: emailBody,
    ...
    >
      ...
      >
        <View className="gap-8 p-4 bg-background/30">
          ...
          >
            <Image
              source={require("~/assets/images/statiktraeger-main.webp")}
              contentFit="contain"
          ...
          <View className="gap-2">
            ...
            <RadioGroup
              ...
              className="gap-3"
            >
              {deckelSetOptions.map((option) => (
                <RadioGroupItemWithLabel
                  key={option}
                  value={option}
                  onLabelPress={() => setDeckelSet(option)}
                />
              ))}
            </RadioGroup>
          </View>
