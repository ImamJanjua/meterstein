...
const Rinne = () => {
  ...
  // Color and gutter type
  const [farbe, setFarbe] = React.useState("");
  const [rinnenart, setRinnenart] = React.useState("");
...
Gesendet über Meterstein
    ...
    try {
      ...
      const result = await MailComposer.composeAsync({
        ...
        subject: `Bestellung - Rinne - ${nameKunde}`,
        body: emailBody,
      ...
      >
        <View className="gap-8 p-4 bg-background/30">
          <View className="mt-8 items-center">
            <Text className="text-3xl font-bold text-red-500">Rinne</Text>
          </View>
          ...
          >
            <Image
              source={require("~/assets/images/rinne-main.webp")}
              contentFit="contain"
          ...
          <View className="gap-2">
            ...
            <RadioGroup
              ...
              className="gap-3"
            >
              {rinnenartOptions.map((option) => (
                <RadioGroupItemWithLabel
                  key={option}
                  value={option}
                  onLabelPress={() => setRinnenart(option)}
                />
              ))}
            </RadioGroup>
          </View>
